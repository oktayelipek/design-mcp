...
- [x] Task 5: Tasarım sistemindeki "okunabilir" kaynakların (Resources) oluşturulan data dosyalarından okunarak tanımlanması (`design-system://tokens/colors`, `design-system://tokens/typography`, `design-system://guidelines/wallet`) → Verify: MCP sunucusunun `resources/list` isteğine doğru yanıt vermesi.
- [x] Task 6: Component bazlı "Tool" ların programlanması (örn: `get_atom_detail`, `get_molecule_detail` vb.) ve data dosyalarından beslenmesi → Verify: MCP sunucusunun `tools/call` ile istenen bileşenin JSON speclerini dönmesi.
- [x] Task 7: LLM'leri yönlendirecek "Prompt"ların eklenmesi (örn: `implement-wallet-part`) → Verify: `prompts/list` isteğine yanıt alınması.
- [x] Task 9: Lokal ortamda Claude Desktop veya mcp-inspector ile SSE üzerinden test edilmesi → Verify: Sunucunun build edilip çalışabilir durumda olması.
- [x] Task 10: Tasarım Rehberleri (Guidelines) desteği eklenmesi → Verify: `get_guidelines` tool'unun ve markdown kaynaklarının çalışması.
